  ...
  describe("Natural Logic Weights") {
    ...
    describe("when a hard assignment") {
      it ("should accept Wordnet monotone jumps") {
        NatLog.hardNatlogWeights.getCount(Evaluator.monoUp_stateTrue(EdgeType.WORDNET_UP)) should be >= -1.0
        NatLog.hardNatlogWeights.getCount(Evaluator.monoDown_stateTrue(EdgeType.WORDNET_DOWN)) should be >= -1.0
      }
      it ("should accept Freebase monotone jumps") {
        NatLog.hardNatlogWeights.getCount(Evaluator.monoUp_stateTrue(EdgeType.FREEBASE_UP)) should be >= -1.0
        NatLog.hardNatlogWeights.getCount(Evaluator.monoDown_stateTrue(EdgeType.FREEBASE_DOWN)) should be >= -1.0
      }
    }
  }

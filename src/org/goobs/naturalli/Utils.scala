...
object EdgeType extends Enumeration {
  ...
  def toMacCartneyRelation(t:EdgeType):String = {
    t match {
      ...
      case QUANTIFIER_REWORD => "≡ (reworded quantifier)"
      case ANGLE_NEAREST_NEIGHBORS => "≡ (nearest neighbors)"
      ...
      case QUANTIFIER_NEGATE => "^ (negated quantifier)"

      // The weird cases...
      case ADD_EXISTENTIAL => "≡ (added existential)"
      case ADD_UNIVERSAL => "≡ (added universal)"
      case ADD_QUANTIFIER_OTHER => "≡ (added quantifier)"
      case DEL_EXISTENTIAL => "≡ (deleted existential)"
      case DEL_UNIVERSAL => "≡ (deleted universal)"
      case DEL_QUANTIFIER_OTHER => "≡ (deleted quantifier)"
      case _ => "?"
    }
  }

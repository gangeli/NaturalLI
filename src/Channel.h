...
// note[Gabor]: This was taken from: https://github.com/cameron314/readerwriterqueue
//              and was originally named readerwriterqueue.h (on 2014-10-12)
...
public:
	...
	// of 2, elements.
	explicit Channel(size_t maxSize = 15)
	...
	AE_FORCEINLINE bool push(T const& element)
	{
		return inner_enqueue<CannotAlloc>(element);
	}
  ...
  template<typename U>
	AE_FORCEINLINE bool poll(U* result) { return try_dequeue(*result); }
...
private:
	...
	struct ReentrantGuard
	{
			...
			: inSection(_inSection)
		{
			assert(!inSection);
			if (inSection) {
				throw std::runtime_error("Channel does not support enqueuing or dequeuing elements from other elements' ctors and dtors");
			}

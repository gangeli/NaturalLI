...
Relation triples produced by
  ...
  are useful for question answering, inference, and other IE tasks.
...
We replace this large pattern set
  with a few patterns for canonically formatted
  sentences, and shift the focus to a classifier 
  which learns to extract self-contained 
  clauses from longer sentences.
We then run natural logic inference over these short
  clauses to determine the maximally specific arguments for each
  candidate triple.
We show that our approach yields a 5\% absolute F$_1$ gain
  over a state-of-the-art open IE system
  on the TAC-KBP 2013 Slot Filling task.
\end{abstract}

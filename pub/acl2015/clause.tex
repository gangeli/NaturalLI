...
From top to bottom, each clause produces a set of entailed shorter
  utterances, and segments those which match an atomic pattern into a
...
At a given node in the parse tree, we classify each outgoing arc
  $e = p \xrightarrow{l} c$, from the governer p to a dependent c with label l,
...
In this case, the correct action is to
  (1) yield the sentence rooted at \ww{born} as a new clauseon the rooted at \ww{born} as a new clause,
  and (2) steal the subject of \ww{took}. \todo{linguistics?}
...
However, for both of the other actions, it is often the case that we would like
  to borrow some portion of the parent tree. \todo{linguistics}

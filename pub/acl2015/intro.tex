...
\begin{figure}[t]
  ...
  \caption{
    ...
    Inference over the sub-structure of arguments, in turn, allows us to
      drop convoluting information (e.g., \ww{of Austria}), but only when
...
Although this is effective at capturing many of these patterns,
  ...
  out-of-domain text.
For instance, while \ww{Obama is president} is extracted correctly 
...
Furthermore, existing systems struggle at producing
  ...
  \ww{Heinz Fischer of Austria} is far less useful for downstream
  applications than simply \ww{Heinz Fischer}.
...
We treat the first stage as a structured prediction task: we traverse
...
Importantly, in many cases \naive ly splitting a clause on a dependency
  edge produces an incoherent utterance (e.g., \ww{Born in Honolulu, Hawaii},
  from \reffig{teaser}) often due to the clause's verb having its subject 
...
A natural framework for solving this problem is natural logic -- 
  a proof system built on the syntax of human language (see Section \refsec{natlog}).
We can then observe that
  \ww{Chancellor Merkel of Germany visited China yesterday} entails
  that \ww{Chancellor Merkel visited China yesterday},
...
For example, \ww{No house cats have rabies} should not entail that
  \ww{cats have rabbis}, or even that \ww{house cats have rabies}.

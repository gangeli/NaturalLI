...
We briefly review natural logic and NaturalLI -- the existing inference
  engine we use.
...
\item This lexical relation between words is projected up to yield a relation between
      sentences, based on the \textit{polarity} of the token.
...
Despite differing by the same lexical relation,
  the sentence-level relation is different in the two cases.
...
From the example above, \w{some} is monotone in its first
  argument (i.e., \w{cat} or \w{animal}), 
  and \w{no} is antitone in its first argument.
This means that the first argument to \w{some} is allowed to mutate 
  up the specified hierarchy (e.g., hypernymy),
  whereas the first argument to \w{no} is allowed to mutate
  down.
...
As a final note, although we refer to the monotonicity calculus
  described above as \ww{natural logic}, this formalism is only 
  one of many possible natural logics.
For example, \newcite{key:1992mcallester-natlog} introduce a syntax for
  first order logic which they call \textit{Montagovian syntax}.
This syntax has two key advantages over first order logic: 
  first, the ``quantifier-free'' version of
  the syntax (roughly equivalent to the monotonicity calculus we use)
  is computationally efficient while still handling limited quantification.
Second, the syntax more closely mirrors that of natural language.
...
NaturalLI casts inference as a search problem: given a hypothesis and an arbitrarily
  large corpus of text, it searches through the space of

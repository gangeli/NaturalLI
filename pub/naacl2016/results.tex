...
To demonstrate the system's robustness on a larger dataset, we additionally
  evaluate on a test set of 250 5$^\textrm{th}$ grade science exams.
...
Nonetheless, the trend illustrated in \reftab{aristonaturalli} holds:
  the standalone classifier scores 43.6\%; 
  Solr alone obtains a score of 46.8\%.
NaturalLI alone scores 46.4\%, which rises to 48.0\% when combined with
  Solr to break ties (at the same ratio of 24:1 for NaturalLI).

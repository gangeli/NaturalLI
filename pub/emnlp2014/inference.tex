...
This approach is efficient over a large database of 300 million entries
  without queries over the database to limit the search space; nor
  does it make use of any sort of approximate matching against 
  the database, beyond lemmatizing individual lexical items.
...
Importantly, this enforces each derivation to order mutations
  right-to-left;
...
Instead, we store the database as a trie.
...
For instance, the path from \textit{cat} to \textit{animal} traverses
  ...
  search depth of 6.
The value $f_{\downarrow_{w \rightarrow w'}}$ is set in the same way.

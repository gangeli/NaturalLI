...
The full table of transition templates is given in \reftab{transitions},
  along with the relation instances of that template introduce.
...
Once we have an edge defining a lexical mutation with a Natural Logic
  relation $r$, we can construct the corresponding end node
  $(w', s')$ such that $w'$ is the sentence with the lexical mutation
  applied, and $s'$ is the validity state obtained from following the
  FSA in \refsec{search} in reverse.
...
We then notice that the transition 
  ...
  inference state, and set our new inference state to be its start --
  in this case, we maintain validity.
...
Note that negation is somewhat subtle, as the transitions are not
  ...
  the edge endpoint's true inference state with respect to the premise
  yet.
In practice, the search procedure treats all three of
  $\{\negate, \alternate, \cover\}$ as negation, and re-score complete
...
We then define $f_{NN_{w \rightarrow w'}}$
  to be the arc cosine of the cosine similarity (i.e., the angel)
...
Note that the size of this vector mirrors the size of number of
...
We can limit the number of items we consider by storing the database
  as a trie.
Since the search mutates the fact left-to-right (as per above), we can

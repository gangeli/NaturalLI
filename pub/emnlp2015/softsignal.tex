...
In fact, this classifier is a stronger baseline than it may seem: evaluating
  the system on RTE-3 \cite{key:2007giampiccolo-rte} yielded 63.75\% accuracy --
  a 2 points above the median submission.
...
A version of the classifier constructed in 
  \refsec{softsignals-classifier}, but over \textit{keywords} rather
...
Note that the lack of longer keyphrases is in general a hinderance to the
...
%  of each other.
Otherwise, any pair of operators which share half their signature are 
...
To illustrate with our example, NaturalLI would mutate \w{no cats have tails}
  ...
  contradictory candidate premise, but which has perfect overlap with the 
